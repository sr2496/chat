<template>
  <div
    class="p-3 bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-700"
  >
    <div class="max-w-4xl mx-auto">
      <div class="flex flex-col gap-2">
        ...
        <div v-if="queuedFiles.length > 0" class="flex flex-wrap gap-2">
          <div
            v-for="(fileObj, index) in queuedFiles"
            :key="index"
          ...
          >
            ...
            >
              <img
                v-if="fileObj.type.startsWith('image/')"
                :src="fileObj.preview"
            ...
            >
              ...
              <div class="min-w-0">
                <p
                  class="font-medium text-gray-900 dark:text-white truncate max-w-24"
                >
                  {{ fileObj.file.name }}
                </p>
              ...
              <button
                @click="removeFile(index)"
                class="ml-1 text-red-500 hover:text-red-600 flex-shrink-0"
              >
                <svg
                  class="w-3 h-3"
                  fill="none"
        ...
        >
          ...
          <button
            ...
            @click="send"
            :disabled="!canSend"
            class="p-2.5 rounded-lg transition-all duration-200 hover:scale-105 shadow-sm"
            :class="
              canSend
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const canSend = computed(
      () => message.value.trim().length > 0 || queuedFiles.value.length > 0
    );
