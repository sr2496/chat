    ...
    <div class="hidden sm:block p-4 bg-chat-surface border-b border-chat-border">
      <div class="max-w-md mx-auto">
        ...
        <div class="relative">
          ...
          />
          <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-chat-text-muted" fill="none" stroke="currentColor"
            viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <transition name="fade">
          <div v-if="!conversationsLoading">
            <div
              ...
              class="flex items-center gap-4 p-3 rounded-lg cursor-pointer hover:bg-chat-bg/50 transition-colors group"
              :class="{
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center justify-between mt-1">
                  <p class="text-sm text-chat-text-muted truncate pr-2">
                    {{ chatStore.typingText(conv.id) || chatStore.getLastMessagePreview(conv.last_message) || 'No messages yet' }}
                  </p>
