<template>
  <Dialog :open="isOpen" @update:open="(v: boolean) => { if (!v) close() }">
    <DialogContent class="max-w-lg p-0 overflow-hidden">
      ...
      <div class="bg-gradient-to-r from-green-500 to-emerald-600 px-6 py-2">
        <div class="flex items-center justify-between text-white">
          <DialogTitle class="text-xl font-bold text-white">Add Members</DialogTitle>
      ...
      <!-- Body -->
      <div class="flex flex-col h-[60vh] sm:h-[400px] bg-white dark:bg-gray-800">
        <!-- Selected Chips -->
        <div v-if="selectedUsers.length > 0" class="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex-shrink-0">
          <div class="flex gap-2 overflow-x-auto custom-scrollbar pb-1">
            <div v-for="user in selectedUsers" :key="user.id"
              class="flex items-center gap-2 bg-green-50 dark:bg-green-900/30 border border-green-100 dark:border-green-800 rounded-full pl-1 pr-3 py-1 flex-shrink-0">
              <UserAvatar :avatar="user.avatar" size="xs" :show-online="false" />
              <span class="text-sm font-medium text-green-800 dark:text-green-300 truncate max-w-[100px]">{{ user.name.split(' ')[0]
              }}</span>
              <button @click="toggleUser(user)"
                class="w-5 h-5 rounded-full hover:bg-red-500 hover:text-white text-green-600 dark:text-green-400 flex items-center justify-center transition">
        ...
        <div class="p-4 border-b border-gray-100 dark:border-gray-700">
          <div class="relative">
            ...
            <input v-model="search" type="text" placeholder="Search people..."
              class="w-full pl-10 pr-4 py-3 rounded-xl bg-gray-50 dark:bg-gray-700 border-none focus:ring-2 focus:ring-green-500/20 text-gray-900 dark:text-gray-100 placeholder-gray-400 transition-all font-medium" />
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-2 py-2 custom-scrollbar">
          ...
          <div v-else class="space-y-1">
            <div v-for="user in filteredUsers" :key="user.id" @click="toggleUser(user)"
              class="flex items-center justify-between p-3 rounded-xl cursor-pointer transition hover:bg-gray-50 dark:hover:bg-gray-700 group select-none">
              ...
              <div class="w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all"
                :class="selectedUsers.some(u => u.id === user.id) ? 'bg-green-500 border-green-500' : 'border-gray-200 dark:border-gray-600 group-hover:border-green-400'">
      ...
      <!-- Footer -->
      <div class="px-6 py-2 bg-gray-50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-3">
        <button @click="close"
          class="px-5 py-2.5 rounded-xl font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
          Cancel
        </button>
...
import type { User, ConversationUser } from '../../types/chat';
...
const props = defineProps<{
  isOpen: boolean;
  groupId: number;
  currentMembers: ConversationUser[];
}>();
