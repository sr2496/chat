    ...
    <Sheet :open="isOpen" @update:open="(v: boolean) => { if (!v) close() }">
        <SheetContent side="right" class="max-w-md flex flex-col">
            ...
            <div class="flex items-center gap-3 p-4 border-b border-gray-100 dark:border-gray-800 sticky top-0 z-10">
                ...
                <SheetTitle class="text-xl font-bold text-chat-text">Contact Info</SheetTitle>
            </div>
...
const contact = ref<ConversationUser | null>(null);
...
const loadContact = async () => {
    ...
    try {
        const conversation = chatStore.conversations.find(c =>
            c.users?.some((u) => u.id === props.userId)
        );

        if (conversation) {
            contact.value = conversation.users?.find((u) => u.id === props.userId);
        }
    } catch (error) {
...
const blockUser = async () => {
    const confirmed = await confirmDialog({
        title: 'Block User',
        message: `Are you sure you want to block ${contact.value?.name || 'this user'}?`,
        confirmText: 'Block',
        variant: 'danger',
    });
    if (confirmed) {
