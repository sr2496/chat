<template>
  <Sheet :open="isOpen" @update:open="(v: boolean) => { if (!v) handleClose() }">
    <SheetContent side="left" class="max-w-sm flex flex-col">
      ...
      <div class="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-4 flex-shrink-0">
        <div class="flex items-center gap-3">
          ...
          <SheetTitle class="text-xl font-bold text-white">New Message</SheetTitle>
        </div>
      </div>
...
export default defineComponent({
  components: { UserAvatar, Sheet, SheetContent, SheetTitle },
  props: {
  ...
  setup(props, { emit }) {
    ...
    const startPrivateChat = async (user: { id: number }) => {
