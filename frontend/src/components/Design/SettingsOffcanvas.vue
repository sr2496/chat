<template>
  <Sheet :open="isOpen" @update:open="(v: boolean) => { if (!v) close() }">
    <SheetContent side="right" class="max-w-sm flex flex-col">
      <!-- Header -->
      <div class="flex items-center justify-between p-6 border-b border-chat-border">
        <SheetTitle class="text-xl font-bold text-chat-text">Settings</SheetTitle>
...
const handleAvatarChange = (event: Event) => {
  ...
  if (file) {
    ...
    if (file.size > 2 * 1024 * 1024) {
      toaster.error('Image must be less than 2MB');
      return;
    }
